#!/usr/bin/env python3
"""
ZTP Bootstrap Script for Arista EOS

Downloaded and executed by Arista switches during Zero Touch Provisioning.
Performs the following:
  1. Checks current EOS version against desired version
  2. Downloads EOS image from HTTP server if version mismatch
  3. Retrieves per-device configuration using the switch serial number
  4. Applies startup configuration and restarts services

NOTE: This file is auto-generated from templates/bootstrap.py.j2
...
# Configuration
# =============================================================================
SERVER_IP = "192.168.130.1"
EOS_IMAGE_DIR = "images"
CONFIG_DIR = "configs"
DESIRED_EOS_VERSION = "4.35.7M"

# =============================================================================
# Logging — syslog + stdout (matches Arista bootstrap pattern)
# =============================================================================
...
def run_cli(cmd_list):
    """Execute EOS CLI commands via FastCli (matches CliManager pattern)."""
    assert os.path.isfile(FAST_CLI_BINARY), "FastCli binary not found"
    cmds = "\n".join(cmd_list)
    try:
...
def get_show_version():
    """Run 'show version | json' and return parsed dict."""
...
def cancel_zerotouch():
    """Delete zerotouch-config and run 'zerotouch cancel' to prevent ZTP loop."""
...
def apply_config_and_restart():
    """Apply startup config by restarting EOS services."""
    log(f"Restarting EOS services in {RESTART_WAIT_TIME} seconds to apply startup configuration...")
    time.sleep(RESTART_WAIT_TIME)
    try:
        subprocess.check_output(
            ["sudo", "service", "ProcMgr", "restart"],
...
def main():
    ...
    if current_version == DESIRED_EOS_VERSION:
        ...
        log("EOS version mismatch, attempting image download...")
        if download_eos_image():
            simulate_boot_image()
        else:
